...
function goToList() {
    ...
    var consultingDescription = document.getElementById('consulting').value;
    var selectedFields = [];

    // 모든 체크박스 요소들을 선택
    var checkboxes = document.querySelectorAll('input[name="consulting_fields[]"]:checked');
    ...
    if (otherCheckbox.checked) {
        // "기타" 옵션의 텍스트 값을 가져옴
        var otherTextValue = document.getElementById('otherField').value;

        // "기타" 옵션의 텍스트 값을 배열에 추가
        selectedFields.push("기타: " + otherTextValue);
    }
    ...
    // 모든 라디오 버튼 요소들을 선택
    var radioButtons = document.querySelectorAll('input[name="whereIn"]:checked');

    // 라디오 버튼이 선택된 경우 해당 값 가져오기
    if (radioButtons.length > 0) {
        selectedWhereIn = radioButtons[0].value;

        // "기타" 옵션이 선택된 경우 텍스트 영역의 값을 가져오기
        if (selectedWhereIn === '기타') {
            var otherField2Value = document.getElementById('otherField2').value;
            // 이제 otherField2Value에 "기타" 옵션의 텍스트 값이 저장됨
            console.log("기타 선택, 텍스트 값: " + otherField2Value);
        }
    }
    ...
    // merchant_uid 매개변수 추출
    var merchant_uid = urlParams.get('merchant_uid');

    // 데이터 합치기
    // formData += '&merchant_uid=' + uid;
    var data = {
        con_name: consultingDescription,
        consulting_fields: selectedFields.join(', '),
        whereIn: selectedWhereIn,
        merchant_uid: merchant_uid
    }
    var xhr = new XMLHttpRequest();
    var url = 'https://merakl.co.kr/api/coupon/merakl/consult'; // 실제 서버 스크립트 경로로 대체
